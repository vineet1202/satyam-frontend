...
const SideBar = ({ links, navState, navStateToggleHandler: handleNavStateToggle }) => {
  ...
  return (
    <FlexCol
      as="aside"
      className={`${navState === "open" && "animate-[slideRight_.3s_ease-out_forwards] shadow-[1px_0_10px_2000px_rgba(13,11,14,.2)]"} top-0 fixed -left-full z-20 h-screen overflow-y-scroll max-w-fit gap-12 bg-white  px-7 pb-32 pt-12 transition-all md:relative  md:left-auto md:animate-none md:border-r-[1px]  md:border-r-gray-300   `}>
      <div
        className={`absolute left-full top-[14%] hidden -translate-x-1/2 rounded-full bg-gray-100 p-2 text-base transition-all   md:block ${navState === "open" ? "rotate-180" : "rotate-0"}`}
        onClick={handleNavStateToggle}>
        <FaAngleRight />
      </div>

      <div className={`${isMobile ? "flex items-start justify-between self-stretch" : "self-center"}`}>
        <Link to="/">
          <Logo type={isMobile || navState === "open" ? "long" : "short"} size={isMobile ? 2.75 : 2.2} />
        </Link>
        {isMobile && <IoClose className="text-3xl text-gray-800" onClick={handleNavStateToggle} />}
      </div>
