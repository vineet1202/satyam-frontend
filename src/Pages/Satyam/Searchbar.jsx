...
const Searchbar = () => {
  ...
  return (
    <Flex className="flex-1 h-10 xsm:h-10 items-center gap-6 pl-3 pr-2 xsm:pr-4 sm:pr-6 xsm:gap-5 xsm:pl-5 sm:gap-6  md:pl-8 ">
      <Flex className=" items-center gap-2 xsm:gap-3 sm:gap-4">
        <FiSearch
          className="text-xl text-[#424242]  xsm:text-2xl sm:text-[1.6rem]"
          onClick={() => setInputState("open")}
        />
        <input
          type="text"
        ...
        />
        {inputState === "open" && (
          <IoClose className="text-2xl ml-auto sm:hidden" onClick={() => setInputState("collapsed")} />
        )}
      </Flex>
